...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%
% Basic simact hierarchy handling
% 
...
%% simact_type(?Task, ?Class) is nondet.
%
...
% 
simact_type(Task, Class) :-
    rdf_has(Task, rdf:type, Class),
    rdf_reachable(Class, rdfs:subClassOf, knowrob:'CRAMEvent').


%% subsimact(?Task, ?Subsimact) is nondet.
%
%  Check if Subsimact is a child simact of Task in the simact tree
%
%  @param Task Identifier of given Task
%  @param Subsimact Identifier of given Subsimact
% 
subsimact(Task, Subsimact) :-
    rdf_has(Task, knowrob:'subAction', Subsimact),
    simact(Task),
    simact(Subsimact).
...
%  @param Subsimact Identifier of given Subsimact
% 
subsimact_all(Task, Subsimact) :-
    owl_has(Task, knowrob:subAction,  Subsimact).
...
%% simact_failure(?Task, ?Failure) is nondet.
%
...
%% cram_holds(+simactPredObj, -simactPredStatus, +T) is nondet.
%
% Check whether the given simact was being continued, done, failed or not yet started at the given time point.
%
% @param Task Identifier of given Task
% @param Status Returned status
% @param T   TimePoint
% 
cram_holds(simact_status(Task, Status), T):-
    nonvar(Task),
    simact(Task),
    simact_start(Task, Start),
    simact_end(Task, End),
...
% 
%     findall(End-Tsk, (
%                 simact_type(Tsk, knowrob:'UIMAPerception'),
%                 simact_end(Tsk, EndTp),
%                 rdf_triple(knowrob:after, EndTp, Time), % only consider simacts that end before Time
...
occurs(object_perceived(Obj),T) :-
    nonvar(Obj),
    nonvar(T),
    simact_type(Task, knowrob:'UIMAPerception'),
    simact_outcome(Task, Obj),
    simact_start(Task, T).


%% simact_used_gripper(+Task, -Link) is nondet.
%
%
...
% 
simact_used_gripper(Task, Link) :-

    subsimact_all(Task, Movement),
    simact_type(Movement, knowrob:'ArmMovement'),
...
publish_designator(Task) :-
    subsimact(Task, Subsimact),
    rdf_has(Subsimact, knowrob:'designator', D),
...
% Get the path of percepted image from the given perception simact
